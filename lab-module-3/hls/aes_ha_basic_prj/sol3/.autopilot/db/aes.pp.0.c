...
# 455 "c_src/aes.c"
  ...
  InvCipher_label8:for (round = (10 - 1);; --round) {
#pragma HLS INLINE
# 465 "c_src/aes.c"
...
void AES_CBC_encrypt_buffer(struct AES_ctx *ctx, uint8_t *buf,
  ...
  uintptr_t i;
  uint8_t *Iv = ctx->Iv;
  AES_CBC_encrypt_buffer_label1:for (i = 0; i < length; i += 16) {
#pragma HLS UNROLL
# 539 "c_src/aes.c"
...
void AES_CBC_decrypt_buffer(struct AES_ctx *ctx, uint8_t *buf,
  ...
  AES_CBC_decrypt_buffer_label0:for (i = 0; i < length; i += 16) {
#pragma HLS UNROLL
# 554 "c_src/aes.c"
...
# 569 "c_src/aes.c"
  ...
  unsigned i;
  int bi;
  AES_CTR_xcrypt_buffer_label2:for (i = 0, bi = 16; i < length; ++i, ++bi) {
#pragma HLS UNROLL
# 574 "c_src/aes.c"

    if (bi == 16)
    {
      _memcpy(buffer, ctx->Iv, 16);
      state_t x;
      ...
      AES_CTR_xcrypt_buffer_label5:for (bi = (16 - 1); bi >= 0; --bi) {
#pragma HLS UNROLL
# 585 "c_src/aes.c"
