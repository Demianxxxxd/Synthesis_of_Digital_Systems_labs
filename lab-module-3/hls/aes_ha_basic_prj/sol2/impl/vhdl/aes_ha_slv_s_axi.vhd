...
architecture behave of aes_ha_slv_s_axi is
    ...
    constant ADDR_INOUT_R_BASE : INTEGER := 16#30#;
    constant ADDR_INOUT_R_HIGH : INTEGER := 16#3f#;
    ...
    signal int_inout_r_address0 : UNSIGNED(1 downto 0);
    signal int_inout_r_ce0     : STD_LOGIC;
    signal int_inout_r_we0     : STD_LOGIC;
    signal int_inout_r_be0     : UNSIGNED(3 downto 0);
    signal int_inout_r_d0      : UNSIGNED(31 downto 0);
    signal int_inout_r_q0      : UNSIGNED(31 downto 0);
    signal int_inout_r_address1 : UNSIGNED(1 downto 0);
    signal int_inout_r_ce1     : STD_LOGIC;
    signal int_inout_r_we1     : STD_LOGIC;
    signal int_inout_r_be1     : UNSIGNED(3 downto 0);
    signal int_inout_r_d1      : UNSIGNED(31 downto 0);
    signal int_inout_r_q1      : UNSIGNED(31 downto 0);
    signal int_inout_r_read    : STD_LOGIC;
    signal int_inout_r_write   : STD_LOGIC;
    signal int_inout_r_shift   : UNSIGNED(1 downto 0);
...
port map (
     ...
     q1       => int_iv_q1);
-- int_inout_r
int_inout_r : aes_ha_slv_s_axi_ram
generic map (
     BYTES    => 4,
     DEPTH    => 4,
     AWIDTH   => log2(4))
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_key_read = '0') and (int_iv_read = '0') and (int_inout_r_read = '0') else '0';
    RVALID    <= RVALID_t;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                ...
                elsif (int_inout_r_read = '1') then
                    rdata_data <= int_inout_r_q1;
                end if;
            end if;
        end if;
    end process;
...
-- ----------------------- Memory logic ------------------
    ...
    -- inout_r
    int_inout_r_address0 <= SHIFT_RIGHT(UNSIGNED(inout_r_address0), 2)(1 downto 0);
    int_inout_r_ce0      <= inout_r_ce0;
    int_inout_r_we0      <= inout_r_we0;
    int_inout_r_be0      <= SHIFT_LEFT(TO_UNSIGNED(1, 4), TO_INTEGER(UNSIGNED(inout_r_address0(1 downto 0))));
    int_inout_r_d0       <= UNSIGNED(inout_r_d0) & UNSIGNED(inout_r_d0) & UNSIGNED(inout_r_d0) & UNSIGNED(inout_r_d0);
    inout_r_q0           <= STD_LOGIC_VECTOR(SHIFT_RIGHT(int_inout_r_q0, TO_INTEGER(int_inout_r_shift) * 8)(7 downto 0));
    int_inout_r_address1 <= raddr(3 downto 2) when ar_hs = '1' else waddr(3 downto 2);
    int_inout_r_ce1      <= '1' when ar_hs = '1' or (int_inout_r_write = '1' and WVALID  = '1') else '0';
    int_inout_r_we1      <= '1' when int_inout_r_write = '1' and WVALID = '1' else '0';
    int_inout_r_be1      <= UNSIGNED(WSTRB);
    int_inout_r_d1       <= UNSIGNED(WDATA);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (ar_hs = '1' and raddr >= ADDR_INOUT_R_BASE and raddr <= ADDR_INOUT_R_HIGH) then
                    int_inout_r_read <= '1';
                else
                    int_inout_r_read <= '0';
                end if;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                if (aw_hs = '1' and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) >= ADDR_INOUT_R_BASE and UNSIGNED(AWADDR(ADDR_BITS-1 downto 0)) <= ADDR_INOUT_R_HIGH) then
                    int_inout_r_write <= '1';
                elsif (WVALID = '1') then
                    int_inout_r_write <= '0';
                end if;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (inout_r_ce0 = '1') then
                    int_inout_r_shift <= UNSIGNED(inout_r_address0(1 downto 0));
                end if;
            end if;
        end if;
    end process;
