...
void _memcpy(uint8_t * dst, const uint8_t* src, unsigned int num)
{
#pragma HLS INLINE
	_memcpy_label9:for (int i = 0; i < num; ++i)
		dst[i] = src[i];
}
...
static void InvMixColumns(state_t *state) {
  int i;
  uint8_t a, b, c, d;
  InvMixColumns_label6:for (i = 0; i < 4; ++i) {
    a = (*state)[i][0];
    b = (*state)[i][1];
    c = (*state)[i][2];
    d = (*state)[i][3];
...
static void InvSubBytes(state_t *state) {
  uint8_t i, j;
  InvSubBytes_label5:for (i = 0; i < 4; ++i) {
    InvSubBytes_label4:for (j = 0; j < 4; ++j) {
...
static void InvCipher(state_t *state, const uint8_t *RoundKey) {
  ...
  InvCipher_label8:for (round = (Nr - 1);; --round) {
    InvShiftRows(state);
    InvSubBytes(state);
    AddRoundKey(round, state, RoundKey);
    if (round == 0) {
...
static void XorWithIv(uint8_t *buf, const uint8_t *Iv) {
  uint8_t i;
  XorWithIv_label3:for (i = 0; i < AES_BLOCKLEN;
...
void AES_CBC_encrypt_buffer(struct AES_ctx *ctx, uint8_t *buf,
  ...
  uintptr_t i;
  uint8_t *Iv = ctx->Iv;
  AES_CBC_encrypt_buffer_label1:for (i = 0; i < length; i += AES_BLOCKLEN) {
    XorWithIv(buf, Iv);
...
void AES_CBC_decrypt_buffer(struct AES_ctx *ctx, uint8_t *buf,
  ...
  AES_CBC_decrypt_buffer_label0:for (i = 0; i < length; i += AES_BLOCKLEN) {
...
void AES_CTR_xcrypt_buffer(struct AES_ctx *ctx, uint8_t *buf, uint32_t length) {
  ...
  unsigned i;
  int bi;
  AES_CTR_xcrypt_buffer_label2:for (i = 0, bi = AES_BLOCKLEN; i < length; ++i, ++bi) {
    if (bi == AES_BLOCKLEN) /* we need to regen xor compliment in buffer */
    {
      _memcpy(buffer, ctx->Iv, AES_BLOCKLEN);
      state_t x;
...
//      Cipher((state_t *)buffer, ctx->RoundKey);
      ...
      AES_CTR_xcrypt_buffer_label5:for (bi = (AES_BLOCKLEN - 1); bi >= 0; --bi) {
        /* inc will overflow */
        if (ctx->Iv[bi] == 255) {
          ctx->Iv[bi] = 0;
          continue;
