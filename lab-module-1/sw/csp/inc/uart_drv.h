...
#ifndef __PERIPH_INC_UART_DRV_H__
#define __PERIPH_INC_UART_DRV_H__
...
{
    /* Registers */
    volatile uint8_t *rbr_; ///< Receiver Buffer Register (make sure LCR.DLAB=0 before access) [RO]
    volatile uint8_t *thr_; ///< Transmitter Holding Register (make sure LCR.DLAB=0 before access) [WO]
    volatile uint8_t *dll_; ///< Divisor Latch (LSByte) Register (make sure LCR.DLAB=1 before access) [RW]
    volatile uint8_t *ier_; ///< Interrupt Enable Register (make sure LCR.DLAB=0 before access) [RW]
    volatile uint8_t *dlm_; ///< Divisor Latch (MSByte) Register (make sure LCR.DLAB=1 before access) [RW]
    volatile uint8_t *iir_; ///< Interrupt Identification Register [RO]
    volatile uint8_t *fcr_; ///< FIFO Control Register [WO]
    volatile uint8_t *lcr_; ///< Line Control Register [RW]
    volatile uint8_t *mcr_; ///< Modem Control Register [RW]
    volatile uint8_t *lsr_; ///< Line Status Register [RO]
    volatile uint8_t *msr_; ///< Modem Status Register [RO]
    volatile uint8_t *scr_; ///< Scratch [RW]
...
#define UART_CREATE_INSTANCE(VAR_NAME, UART_METHODS_HANDLE_PTR, BASE_ADDR) \
    uart_t VAR_NAME = { .rbr_ = (void *)(BASE_ADDR + UART_OFFSET_RBR),     \
...
#endif /*__PERIPH_INC_UART_DRV_H__*/
