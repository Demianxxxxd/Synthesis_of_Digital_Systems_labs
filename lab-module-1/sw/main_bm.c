...
void uart_send_string(const char *str)
{
    ...
    */
    //my code_Jian Li_12/05/2023
    volatile uint8_t *lsr = (void *)(UART_BASE_ADDR + UART_OFFSET_LSR);
    int i=0;
    while(buf[i] !='\0') // when still writing into thr
    {
    	while(1) // dead loop until break,write new char
    	{
    		if(*lsr & 0b00100000) break;//lsr 5.bit set
    	}
		*thr =buf[i];
		i++;
    }
